...
public class BubbleSortApp {
    public static void main(String[] args) {
        Map<String, Medalhista> medalhistas = carregarMedalhistas("C:\\Users\\arthu\\OneDrive\\Desktop\\All\\my\\software_engineering\\Atividades\\3° Periodo - 2024\\AEDS II - Medallists\\src\\tmp\\medallists.csv");
        ordenarMedalhistas(medalhistas);
        processarEntradaUsuario(medalhistas);
    }
    ...
    private static void ordenarMedalhistas(Map<String, Medalhista> medalhistas) {
        Bubblesort<Medalhista> ordenator = new Bubblesort<>();
        Medalhista[] medalhistasArray = medalhistas.values().toArray(new Medalhista[0]);
        ordenator.ordenar(medalhistasArray);

        medalhistas.clear();
        for (Medalhista medalhista : medalhistasArray) {
            medalhistas.put(medalhista.getNome(), medalhista);
        }
    }

    private static void processarEntradaUsuario(Map<String, Medalhista> medalhistas) {
    ...
    private static void exibirRelatorioMedalhista(Medalhista medalhista) {
        ...
        int quantidadeOuro = medalhista.contarMedalhasPorTipo(TipoMedalha.OURO);
        int quantidadePrata = medalhista.contarMedalhasPorTipo(TipoMedalha.PRATA);
        int quantidadeBronze = medalhista.contarMedalhasPorTipo(TipoMedalha.BRONZE);

        if (quantidadeOuro > 0) {
            System.out.println("Quantidade de medalhas de ouro: " + quantidadeOuro);
        }
        if (quantidadePrata > 0) {
            System.out.println("Quantidade de medalhas de prata: " + quantidadePrata);
        }
        if (quantidadeBronze > 0) {
            System.out.println("Quantidade de medalhas de bronze: " + quantidadeBronze);
        }
...
class Bubblesort<T> implements IOrdenator<T> {
    ...
    public void ordenar(T[] elementos) {
        for (int i = 0; i < elementos.length - 1; i++) {
            for (int j = 0; j < elementos.length - 1 - i; j++) {
                if (compare((Medalhista) elementos[j], (Medalhista) elementos[j + 1]) > 0) {
                    T temp = elementos[j];
    ...
    private int compare(Medalhista m1, Medalhista m2) {
        int ouroCompare = Integer.compare(m2.contarMedalhasPorTipo(TipoMedalha.OURO),
                m1.contarMedalhasPorTipo(TipoMedalha.OURO));
        if (ouroCompare != 0)
            return ouroCompare;

        int prataCompare = Integer.compare(m2.contarMedalhasPorTipo(TipoMedalha.PRATA),
                m1.contarMedalhasPorTipo(TipoMedalha.PRATA));
        if (prataCompare != 0)
            return prataCompare;

        int bronzeCompare = Integer.compare(m2.contarMedalhasPorTipo(TipoMedalha.BRONZE),
                m1.contarMedalhasPorTipo(TipoMedalha.BRONZE));
        if (bronzeCompare != 0)
            return bronzeCompare;
